...
public class DatabaseManager {
    ...
    public static long createGroup(String title, byte[] avatar, long creatorId, List<Long> memberIds) {
        String createConvSql = "INSERT INTO conversations (id, type, title, creator_id, picture) VALUES (?, 'group', ?, ?, ?)`";
...
// 1) conversations
                ...
                if (avatar != null) psCreate.setBytes(4, avatar); else psCreate.setNull(4, Types.BLOB);
                psCreate.executeUpdate();


// 2) participants — ensure creator is present and admin
                boolean creatorPresent = false;
                for (Long uid : memberIds) if (uid != null && uid == creatorId) { creatorPresent = true; break; }
                if (!creatorPresent) memberIds.add(0, creatorId);
                ...
                for (Long uid : memberIds) {
                    ...
                    psAdd.setLong(2, uid);
                    psAdd.setString(3, (uid == creatorId) ? "admin" : "member");
                    psAdd.addBatch();
                }
                psAdd.executeBatch();
                ...
                conn.rollback();
                logSql("createGroup", e);
            }
        } catch (SQLException e) {
            logSql("transactionSetupGroup", e);
        }
        return -1;
    }
